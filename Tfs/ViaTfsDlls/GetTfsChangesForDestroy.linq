...
void Main()
{
	// find check-ins and create destroy links
	var anyUser = false;
	...
	tfsChanges
		.Select(cs=>new{ cs.Owner,
			ChangeSetId=cs.ChangesetId.ToString(),
			cs.CreationDate, 
			Changes=cs.Changes.Select(c=>new { Item = c, Destroy = new Hyperlinq(()=> Util.Cmd(@"C:\Program Files (x86)\Microsoft Visual Studio 12.0\Common7\IDE\TF.exe","destroy /noprompt " + c),"destroy " + c)})
