...
if(toAdd.IsNullOrEmpty() || System.IO.Directory.Exists(toAdd)==false)
	return;
pathVars= oldPath.Split(';').Prepend(toAdd).ToList().Select(p=>new{p,Exists= System.IO.Directory.Exists(p)}).Dump();
