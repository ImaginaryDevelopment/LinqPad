...
type Coord = { Lon: decimal; Lat:decimal}
	with 
	member x.GetGoogleMapLink() = 
		sprintf "http://maps.google.com/?q=%f,%f" x.Lat x.Lon
...
let getFromApiLatLong debug (key:string) lang lat long = 
	use client=new HttpClient()
	client.DefaultRequestHeaders.Add("X-Mashape-Key",key);
	let uri = sprintf "https://community-open-weather-map.p.mashape.com/weather?lang=%s&lat=%f&lon=%f" lang lat long
	uri.Dump("uri")
...
let byLatLong = toTypedResult true <| getFromApiLatLong true (Util.GetPassword("X-Mashape-Key")) "en" 34.75 -77.43
