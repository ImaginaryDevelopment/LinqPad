                ...
                | :? Linq.JObject as jo ->
                    ...
                    | Some pRaw when pRaw.PropertyType = typeof<string> -> 
                        if isRecord then
                            ...
                            let nextProps = props |> Seq.map(fun p -> if p=pRaw then p.GetValue(result) else box raw) |> Array.ofSeq
                            printfn "Properties = 
...
module YouTubes = 
    ...
    type PlaylistIdContainer = {Kind:string; ETag:string; PageInfo:PageInfo; Items:PlaylistIdentifier list; Raw:string } // might be that list is only working because there was a single item
...
let getChannels () = // includes playlists? maybe?
    getCachedData "channels" (sprintf "/youtube/v3/channels?part=id&forUsername=YamahaB&key=%s" apiKey)
    |> (fun raw -> raw|> deserialize2<PlaylistIdContainer> |> fun x -> {x with Raw=raw})
...
let getWholeList listId = 
    ...
    |> Seq.unfold(fun (pState:PagingState) ->
        ...
        | Progress (Some t) ->
            //printfn "Getting page %i" pageIndex
        ...
        |> Option.map(fun x ->
            ...
            let toDisplay (y:PlaylistItem) = 
                ...
                {Title=y.Snippet.Title; Position=y.Snippet.Position; PublishedAt=y.Snippet.PublishedAt;Image=Util.Image(y.Snippet.Thumbnails.Default.Url); ShortDescription=sd; Description=d; Raw=Util.OnDemand("Raw", null)}
            let itemized = x.Items |> Seq.map toDisplay |> List.ofSeq
...
//|> tee dumpReverse
|> Seq.map(deserializeObjectReflectively<ChannelPlaylistContainer>)
