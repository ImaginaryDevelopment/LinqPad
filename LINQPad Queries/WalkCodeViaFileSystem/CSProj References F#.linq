...
module preferences = 
    ...
    let interestedRefName = Some "ValueTuple"
module Seq = 
...
//		| None -> false
    let (|IncludeEndsWith|_|) ending xe = 
        let getEndsWith x = endsWithC ending StringComparison.InvariantCultureIgnoreCase x
        match getAttribValueOrNone "include" xe |> Option.map getEndsWith with
        | Some true -> Some()
        | _ -> None
...
let csProjects=
    projects
    |> Seq.filter (fun f->(f.EndsWith(".csproj") || f.EndsWith(".fsproj")) && f.Contains("test", StringComparison.InvariantCultureIgnoreCase)=false //don't check testing projects
...
// we need to keep the xelement so we can include hint paths
let hasInclude (x:XElement) = getAttribValueOrNone "Include" x |> Option.isSome
...
let references= seq { for i in baseItems do
                        for itemGroup in i.ProjNode.Elements(i.RootNs+"ItemGroup") do
                            let allIncludes : XElement[] = itemGroup.Nodes() |> Seq.ofType<XElement> |> Seq.filter hasInclude |> Array.ofSeq
                            
                            // xaml files can do their own referencing, I think
                            let xaml = if includeAllXaml then allIncludes |> Seq.choose (function | IncludeEndsWith "xaml" as x -> Some x | _ -> None) else Seq.empty
...
(
    ...
    let hasInclude (x:XElement,name):bool = getAttribValueOrNone "Include" x |> Option.bind (fun v -> optFromBool <| v.Contains(name))  |> Option.isSome
