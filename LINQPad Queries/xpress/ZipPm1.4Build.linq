...
let targetDir = @"C:\TFS\PracticeManagement\dev\PracticeManagement-branch\bin"
...
let targetFilename bvi = 
    sprintf "Pm_1.4.%02i_%i%02i%i_%i.zip" bvi.Subversion bvi.Year bvi.Month bvi.Day bvi.BuildNumber
    
let outputDir = @"\\fs01\builds\PM\Pm_VsBuild_Release_Deploy"
...
let latest = 
    ...
    |> function
        | None -> None 
        | Some h -> 
            Regex.Match(h, @"Pm_1.4.([0-9]+)_([0-9]{4})([0-9]{2})([0-9]{2})_([0-9]+)\.zip")
            |> fun m -> {
