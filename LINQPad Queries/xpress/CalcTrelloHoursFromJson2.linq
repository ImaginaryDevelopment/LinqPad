...
let actionStream =
    parsed.actions
    |> Seq.map(fun x -> 
        match x.data.card with
        | None -> "null", "null"
        | Some card -> card.name,card.id
        |> fun (n,i) -> n,i, x.date, x.memberCreator.username, x.data.listBefore |> Option.map(fun lb -> lb.name), x.data.listAfter |> Option.map(fun lb -> lb.name)
    )
    
let getCard cardId = 
    parsed.cards |> Seq.tryFind(fun c -> c.id = cardId)
type CardMove2 = {Dt:DateTime; From:string; To:string; Mover:string; RawDt:string}
type CardInfo= {Name:string; CardStatus: string; Moves: CardMove2 list}  
let getCardStatus cardId =
    getCard cardId
    |> Option.bind(fun card -> listIdMap |> Map.tryFind card.idList)
    |> Option.defaultValue null
    
(Map.empty,actionStream)
||> Seq.fold(fun m (n,i,dt,mover,f,t) ->
    let mv = {  Dt= DateTime.Parse dt;Mover=mover;From= f |> Option.defaultValue "";To= t |> Option.defaultValue "";RawDt=dt}
    match Map.tryFind i m with
    | None -> m |> Map.add i {Name=n; CardStatus= getCardStatus i; Moves=[mv]}
    | Some l -> m |> Map.add i {l with Moves= mv::l.Moves}
)
|> Map.filter(fun _ x ->
    x.Moves |> Seq.exists(fun m ->
        m.Dt.Month = 7 || m.Dt.Month = 8
    )
    && (canBill |> Set.contains(x.CardStatus))
)
|> Map.map(fun _ x ->
    {x with Moves = x.Moves
                    |> List.filter(
                        function
                        | {From=ValueString _} -> true
                        | {To=ValueString _} -> true
                        | _ -> false)
    }
)
|> fun x -> x.Dump()
...
let toBill =
    canBill
    |> Set.map(fun n -> listNameMap.[n])
    //listNameMap.["Unpaid+Done"]
//let excludingCards = parsed.cards |> Seq.filter(fun c -> c.idList <> paidId)
let includingCards = parsed.cards |> Seq.filter(fun c -> toBill |> Set.contains( c.idList )) // = unpaidId)
...
let focusedMap =
    ...
    |> Seq.choose(fun act -> //Option.isSome act.data.listBefore 
        let createDetail (cca:CardChangeAct) =
            ...
            Some {Name=act.data.card |> Option.map(fun d -> d.name) |> Option.defaultValue "null";Detail=cdd}
        match act with    
