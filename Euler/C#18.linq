...
var	targetTree=tree;
	...
	var highestPath = trees.Where(a=>a.SumDescending==max || (a.Left!=null && a.Left.SumDescending==max) || a.Right!=null && a.Right.SumDescending==max).Dump();
...
IEnumerable<BTree> BuildTree(BTree parent,int depth,int column, IEnumerable<string> leaves){
	...
	var siblings= leaves.First().Split(' ').Select((a,i)=>new{Column=i,Value=int.Parse( a.ToString())});
	var children=leaves.Skip(1);
	
	BTree result;
	if(siblings.Count()==1 && parent==null){
		//top of tree
		var node=siblings.Single().Dump("tree top");
		...
		foreach(var l in leaflets){
			...
			leaf.SumDescending+=parent.SumDescending;
			if(parent.Left==null)
				parent.Left=leaf;
			else
				parent.Right=leaf;
