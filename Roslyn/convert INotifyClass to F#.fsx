...
module ScriptOptions = 
    ...
    let isDebugPropPred name = if name = "StatementPhoneNumber" then Yes else No
...
let rec mapNode (memberNames:Set<string>) printDebug (node:SyntaxNode) =
    ...
    | :? IfStatementSyntax as ifss -> 
        ...
        let elseBlock = 
            if ifss.Else <> null then 
                let elseBlock = mapNodeP ifss.Else
                // TODO: ifss.Else
                if String.IsNullOrEmpty(elseBlock) then
                    Some elseBlock
                else None
            else None
        "IfStatementSyntax", sprintf "if %s then %s%s" (mapNodeP ifss.Condition) statements (if elseBlock.IsSome then sprintf "else %s" elseBlock.Value else String.Empty)
